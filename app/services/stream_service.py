...
class StreamService:
    ...
    def add_to_queue(self, audio_path: str, duration: float = 0, metadata: dict | None = None):
        ...
        _stream_state["version"] += 1
        logger.info(f"Stream queue: +{os.path.basename(audio_path)} ({duration:.1f}s) at front, total={len(self.play_queue)}")
    ...
    def reset_position(self):
        ...
        if self.play_queue:
            ...
            if fp and os.path.exists(fp):
                _stream_state["current_file"] = fp
                _stream_state["position"] = 0
                _stream_state["total_bytes"] = os.path.getsize(fp)
                _stream_state["started_at"] = time.time()
                _stream_state["_looping"] = False
                logger.info(f"Stream reset: {os.path.basename(fp)}")
        elif _stream_state.get("_last_file"):
            last = _stream_state["_last_file"]
