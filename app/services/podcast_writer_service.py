...
def build_podcast_writer_graph() -> StateGraph:
    """构建播客转译 LangGraph 子图：
    write_scripts → synthesize_audio → process_audio → END
    """
    workflow = StateGraph(PodcastWriterState)

    workflow.add_node("write_scripts", write_scripts)  # type: ignore[arg-type]
    ...
    workflow.set_entry_point("write_scripts")
    workflow.add_edge("write_scripts", "synthesize")
